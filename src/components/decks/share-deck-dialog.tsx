...
export function ShareDeckDialog({
  ...
  const handleUpdatePermission = async (
    shareId: string,
    newPermission: 'view' | 'collaborate',
  ) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(
          result.error || t('deck.shareDialog.failedToUpdatePermission'),
        );
      }
    } catch {
  ...
  return (
    <>
        ...
        <DialogContent className="sm:max-w-[525px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="username">
                {t('deck.shareDialog.usernameLabel')}
              </Label>
              <Input
                id="username"
            ...
            <div className="grid gap-2">
              <Label htmlFor="permission">
                {t('deck.shareDialog.permissionLabel')}
              </Label>
              <Select
                value={permission}
                onValueChange={(value) =>
                  setPermission(value as 'view' | 'collaborate')
                }
              >
                ...
                <SelectContent>
                  <SelectItem value="view">
                    {t('deck.shareDialog.viewOnly')}
                  </SelectItem>
                  <SelectItem value="collaborate">
                    {t('deck.shareDialog.collaborate')}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          {shares.length > 0 && (
            <div className="border-t pt-4">
              <h4 className="mb-3 text-sm font-medium">
                {t('deck.shareDialog.currentShares')}
              </h4>
                  ...
                  >
                    <div className="flex-1">
                      <p className="text-sm font-medium">
                        {share.username || share.sharedWithUserId}
                      </p>
                      <div className="mt-1">
                        <Select
                    ...
                    >
                      {removingShareId === share.id
                        ? t('deck.shareDialog.removing')
                        : t('deck.shareDialog.remove')}
                    </Button>
                  </div>
                ))}
              </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" onClick={handleShare} disabled={isSubmitting}>
              {isSubmitting
                ? t('deck.shareDialog.sharing')
                : t('deck.shareDialog.share')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog
        open={!!shareToRemove}
        onOpenChange={(open) => !open && setShareToRemove(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('deck.shareDialog.removeShareTitle')}
            </AlertDialogTitle>
            <AlertDialogDescription>
