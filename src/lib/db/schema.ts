import { pgTable, text, timestamp, pgEnum, uuid, unique, index, integer, boolean, jsonb, real } from 'drizzle-orm/pg-core';
...
export const gameStatusEnum = pgEnum('game_status', ['lobby', 'playing', 'ended', 'abandoned']);
export const roundStatusEnum = pgEnum('round_status', ['playing', 'judging', 'completed']);
...
export const gameSession = pgTable('GameSession', {
  ...
  settings: jsonb('settings').notNull().$type<{ pointsToWin: number; handSize: number; hasTimer: boolean; timerSeconds?: number }>(),
 ...
 */
export const gameSessionDeck = pgTable('GameSessionDeck', {
  id: uuid('id').defaultRandom().primaryKey(),
  sessionId: uuid('session_id').notNull().references(() => gameSession.id, { onDelete: 'cascade' }),
  deckId: uuid('deck_id').notNull().references(() => deck.id, { onDelete: 'cascade' }),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
...
export const round = pgTable('Round', {
  ...
  blackCardId: uuid('black_card_id').notNull().references(() => card.id),
  czarPlayerId: uuid('czar_player_id').notNull().references(() => player.id),
 ...
 */
export const completedGame = pgTable('CompletedGame', {
  id: uuid('id').defaultRandom().primaryKey(),
  ...
  winnerUserIdIdx: index('CompletedGame_winner_user_id_idx').on(table.winnerUserId),
  completedAtIdx: index('CompletedGame_completed_at_idx').on(table.completedAt),
 ...
 */
export const completedGameDeck = pgTable('CompletedGameDeck', {
  id: uuid('id').defaultRandom().primaryKey(),
  completedGameId: uuid('completed_game_id').notNull().references(() => completedGame.id, { onDelete: 'cascade' }),
  deckId: uuid('deck_id').notNull().references(() => deck.id, { onDelete: 'cascade' }),
}, (table) => ({
  completedGameIdIdx: index('CompletedGameDeck_completed_game_id_idx').on(table.completedGameId),
 ...
 */
export const completedGamePlayer = pgTable('CompletedGamePlayer', {
  id: uuid('id').defaultRandom().primaryKey(),
  completedGameId: uuid('completed_game_id').notNull().references(() => completedGame.id, { onDelete: 'cascade' }),
  clerkUserId: text('clerk_user_id'), // Null for guest players
  nickname: text('nickname').notNull(),
  ...
  completedGameIdIdx: index('CompletedGamePlayer_completed_game_id_idx').on(table.completedGameId),
  clerkUserIdIdx: index('CompletedGamePlayer_clerk_user_id_idx').on(table.clerkUserId),
}));
 ...
 */
export const completedRound = pgTable('CompletedRound', {
  id: uuid('id').defaultRandom().primaryKey(),
  completedGameId: uuid('completed_game_id').notNull().references(() => completedGame.id, { onDelete: 'cascade' }),
  roundNumber: integer('round_number').notNull(),
  blackCardId: uuid('black_card_id').notNull().references(() => card.id),
  ...
  allSubmissions: jsonb('all_submissions').notNull().$type<Array<{ playerId: string; nickname: string; cardIds: string[] }>>(),
  completedAt: timestamp('completed_at', { withTimezone: true }).notNull(),
}, (table) => ({
  completedGameIdIdx: index('CompletedRound_completed_game_id_idx').on(table.completedGameId),
}));
 ...
 */
export const playerStatistic = pgTable('PlayerStatistic', {
  ...
  favoriteWinningCards: jsonb('favorite_winning_cards').notNull().default([]).$type<Array<{ cardId: string; winCount: number }>>(),
  lastPlayedAt: timestamp('last_played_at', { withTimezone: true }),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
  winRateIdx: index('PlayerStatistic_win_rate_idx').on(table.winRate),
  totalGamesWonIdx: index('PlayerStatistic_total_games_won_idx').on(table.totalGamesWon),
  lastPlayedAtIdx: index('PlayerStatistic_last_played_at_idx').on(table.lastPlayedAt),
}));
...
export const gameSessionDeckRelations = relations(gameSessionDeck, ({ one }) => ({
...
export const completedGameDeckRelations = relations(completedGameDeck, ({ one }) => ({
...
export const completedGamePlayerRelations = relations(completedGamePlayer, ({ one }) => ({
  completedGame: one(completedGame, {
